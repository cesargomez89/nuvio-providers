 ...
 * Generated: 2026-05-04T01:14:26.842Z
 */
...
function extractStreams(tmdbId, mediaType, season, episode, title) {
  return __async(this, null, function* () {
    if (!tmdbId)
      return [];
    console.log(`[CineCity] Iniciando b\xFAsqueda: ${tmdbId} - ${mediaType}`);
    ...
    try {
      ...
      const searchHtml = yield (0, import_http.fetchHtml)(searchUrl, { headers: HEADERS }).catch(() => null);
      if (!searchHtml)
        return [];
      ...
      if (!mediaUrl) {
        const homeHtml = yield (0, import_http.fetchHtml)(MAIN_URL, { headers: HEADERS }).catch(() => null);
        if (homeHtml) {
      ...
      if (!mediaUrl) {
        console.log(`[CineCity] No se encontr\xF3 la URL para: ${searchTitle}`);
        return [];
      }
      console.log(`[CineCity] Procesando enlace: ${mediaUrl}`);
      const pageHtml = yield (0, import_http.fetchHtml)(mediaUrl, { headers: HEADERS }).catch(() => null);
      if (!pageHtml)
        return [];
      ...
      $page("script").each((i, el) => {
        ...
        if (htmlStr && htmlStr.includes("atob")) {
          ...
          while ((match = regex.exec(htmlStr)) !== null) {
            try {
              const decoded = import_crypto_js.default.enc.Base64.parse(match[2]).toString(import_crypto_js.default.enc.Utf8);
      ...
      const addStream = (url, streamTitle, qualityLabel) => {
        ...
        streams.push({
          ...
          headers: __spreadProps(__spreadValues({}, HEADERS), { "User-Agent": currentUA }),
          verified: true
        });
      };
      ...
      console.log(`[CineCity] Total enlaces extra\xEDdos: ${streams.length}`);
      return yield (0, import_engine.finalizeStreams)(streams, "CineCity", searchTitle);
    } catch (error) {
      console.error(`[CineCity] Error cr\xEDtico: ${error.message}`);
      return [];
    }
  });
}
