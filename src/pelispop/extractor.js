...
function fetchWithTimeout(url, options = {}) {
    ...
    return fetchHtml(url, hasAbort ? { ...options, signal: controller.signal } : options)
...
async function getSeriesEmbedUrls(seriesUrl, season, episode) {
    try {
        ...
        const episodeUrl = (season != null && episode != null)
...
export async function extractStreams(tmdbId, mediaType, season, episode, title) {
    ...
    try {
        ...
        if (!selectedUrl) {
            console.log(`[PelisPop] No se encontró${isMovieType ? " la película" : " la serie"}: ${mediaTitle}`);
            return [];
        }
