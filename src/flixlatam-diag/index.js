async function getStreams(tmdbId, mediaType, season, episode) {
  ...
  try {
    ...
    else out.push(diagStream(1, 'CryptoJS-BadHash'));
  } catch (e) {
  ...
  try {
    ...
    else out.push(diagStream(2, 'Cheerio-BadParse'));
  } catch (e) {
    out.push(diagStream(2, 'Cheerio-Fail'));
  }

  // Step 4 — HTTP fetch to flixlatam.com
  try {
    ...
    else out.push(diagStream(3, 'FetchFlix-Status' + resp.status));
  } catch (e) {
  ...
  try {
    ...
    else out.push(diagStream(4, 'Search-Status' + s.status));
  } catch (e) {
    out.push(diagStream(4, 'Search-Error'));
  }

  // Step 5b — TMDB title lookup (as used by flixlatam)
  try {
    ...
    else out.push(diagStream(5, 'TMDB-Status' + tmdbResp.status));
  } catch (e) {
    out.push(diagStream(5, 'TMDB-Error'));
  }

  // Step 6 — Vidurl page + PoW params
  try {
    ...
    for (var n = 0; n < MAX_ITER; n++) {
      ...
      if (h.startsWith(prefix)) { solved = true; out.push(diagStream(7, 'PoW-OK-n' + n)); break; }
    }
    if (!solved) { out.push(diagStream(7, 'PoW-Exhausted')); return out; }

    // Step 8 — Decrypt + embeds
    var dataLink;
    ...
    for (var item of dataLink) {
      ...
      for (var emb of item.sortedEmbeds) {
        ...
        try {
          ...
          if (decUrl && decUrl.startsWith('http')) latCount++;
        } catch (e) {}
      }
    }
