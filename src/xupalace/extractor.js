...
function getXuSlugs(imdbId, title) {
  ...
  if (title) {
    ...
    const fullSlug = titleUpper
      .replace(/[^A-Z0-9]/g, '-')
      .replace(/-+/g, '-')
      .replace(/^-|-$/g, '');
    if (fullSlug) variants.push(fullSlug);
    if (titleUpper.startsWith('THE ')) {
      const noThe = titleUpper
        .replace('THE ', '')
        .replace(/[^A-Z0-9]/g, '-')
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '');
      if (noThe) variants.push(noThe);
    }
    const firstWord = titleUpper.split(' ')[0].replace(/[^A-Z0-9]/g, '');
    if (firstWord && firstWord !== 'THE') variants.push(firstWord);
  }
