...
function extractSvLinks(html) {
  ...
  for (const entry of entries) {
    try {
      ...
      const name = ((entry.match(/name\s*:\s*["']([^"']+)["']/) || [])[1] || 'FC')
        .replace(/&#9989;/g, '')
        .replace(/&amp;/g, '&')
        .replace(/✅/g, '')
        .trim();
...
export async function extractStreams(tmdbId, mediaType, season, episode, title) {
  try {
    ...
    const searchTitle = isMovie
      ? cleanTitle
      : mediaType === 'tv' && season
        ? `${cleanTitle} ${season}x${String(episode).padStart(2, '0')}`
        : cleanTitle;
    ...
    if (entries.length === 0 && isMovie && mediaYear) {
      ...
      const yearJson = await fetchJson(yearSearchUrl, {
        headers: { 'User-Agent': DEFAULT_HEADERS['User-Agent'] },
      });
      ...
      const fallbackEntries = entries
        .filter((e) => {
          ...
          if (isMovie && mediaYear) {
            ...
            const wordMatch =
              firstWord.length > 2 &&
              (firstWord.includes(fFirstWord) || fFirstWord.includes(firstWord));
    ...
    const resolutionResults = await Promise.allSettled(
      sortedLinks.map(async (link) => {
        ...
        if (result && result.url) {
          const finalQuality =
            link.quality?.includes('1080') || link.quality?.includes('FHD')
              ? '1080p'
              : result.quality || link.quality || '720p';
          return {
            langLabel: 'Latino',
