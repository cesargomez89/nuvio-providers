...
export async function extractStreams(tmdbId, mediaType, season, episode, providedTitle) {
  ...
  try {
    ...
    if (!movieMatch) movieMatch = searchData.results.find((r) => r.type === targetType);
    if (!movieMatch || !movieMatch.slug) return [];
    ...
    let embeds = [];
    if (mediaType === 'movie') {
      if (playersData && playersData.embeds) embeds = playersData.embeds;
    } else {
    ...
    const streamPromises = embeds.map(async (player) => {
      ...
      if (lang.includes('sub') || lang.includes('vose') || lang.includes('espana')) return null;
