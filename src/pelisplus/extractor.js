  ...
  try {
    ...
    const searchPromises = titlesToTry.slice(0, 3).map(async (t) => {
      try {
        ...
        while ((m = aRegex.exec(html)) !== null) {
          ...
          const isUrlMatch =
            href.toLowerCase().includes(t.toLowerCase().replace(/\s+/g, '-')) ||
            (tWords.length > 0 && tWords.every((w) => href.toLowerCase().includes(w)));

          if (
            titleMatch(t, resultTitle) ||
            titleMatch(title, resultTitle) ||
            isWordMatch ||
            isUrlMatch
          ) {
    ...
    if (rawResults.length === 0) {
      ...
      $('li[data-url]').each((i, el) => {
        ...
        if (
          sUrl &&
          (sName.toLowerCase().includes('latino') ||
            ($(el).attr('title') || '').toLowerCase().includes('latino'))
        ) {
    ...
    const processStream = async (res) => {
      ...
      try {
        ...
        if (!embedUrl && res.serverId) {
          const embedRes = await fetchJson(
        ...
        const headers =
          typeof finalUrl === 'object' && finalUrl.headers
            ? finalUrl.headers
            : { Referer: BASE_URL };
