...
function normalizeServer(server, url = '', resolvedServerName = null) {
  ...
  if (u.includes('goodstream') || s.includes('goodstream')) return 'GoodStream';
  if (isMirror(u, 'FASTREAM') || isMirror(s, 'FASTREAM')) return 'Fastream';
  if (isMirror(u, 'DROPCDN') || isMirror(s, 'DROPCDN')) return 'DropCDN';
  if (u.includes('vimeos') || u.includes('vms.sh') || s.includes('vimeos')) return 'Vimeos';
  ...
  if (url && url.includes('supervideo')) return 'Supervideo';
  if (isMirror(u, 'DOODSTREAM') || isMirror(s, 'DOODSTREAM')) return 'DoodStream';
  if (url) {
    try {
...
async function finalizeStreams(streams, providerName) {
        ...
        try {
          ...
          if (s.url && (s.url.includes('.m3u8') || s.url.includes('.mp4'))) {
            ...
            try {
              let validated;
              if (hasAbort) {
                validated = await validateStream(s, controller.signal);
              } else {
                validated = await Promise.race([
  ...
  for (const s of validatedStreams) {
    ...
    if (!isLatino && providerName !== 'FuegoCine') continue;
